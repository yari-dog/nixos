#!/usr/bin/env bash

set +e

# JS field injection code from https://github.com/qutebrowser/qutebrowser/blob/main/misc/userscripts/password_fill
javascript_escape() {
    # print the first argument in an escaped way, such that it can safely
    # be used within javascripts double quotes
    # shellcheck disable=SC2001
    sed "s,[\\\\'\"\/],\\\\&,g" <<<"$1"
}

js() {
    cat <<EOF
    function isVisible(elem) {
        var style = elem.ownerDocument.defaultView.getComputedStyle(elem, null);
        if (style.getPropertyValue("visibility") !== "visible" ||
            style.getPropertyValue("display") === "none" ||
            style.getPropertyValue("opacity") === "0" ||
            elem.disabled) {
            return false;
        }
        return elem.offsetWidth > 0 && elem.offsetHeight > 0;
    };
    function isPasswordField(elem) {
        return (elem.type == "password" || elem.autocomplete == "current-password");
    };
    function isUsernameField(elem) {
        return (isVisible(elem) &&
               ((elem.type == "text" ||
                 elem.type == "email") &&
                (elem.autocomplete == "email" ||
                 elem.autocomplete == "username" ||
                 elem.name == "email" ||
                 elem.name == "username")))
    };
    function hasPasswordField(form) {
        var inputs = form.getElementsByTagName("input");
        for (var j = 0; j < inputs.length; j++) {
            var input = inputs[j];
            if (isPasswordField(input)) {
                return true;
            }
        }
        return false;
    };
    function inputToField(input, value) {
        input.click();
        input.focus();
        input.dispatchEvent(new Event("input", {"bubbles": true}));
        input.dispatchEvent(new Event('change', {"bubbles": true}));
        input.value = value;
        input.dispatchEvent(new Event("input", {"bubbles": true}));
        input.dispatchEvent(new Event('change', {"bubbles": true}));
    };
    function loadData2Form (inputs) {
        for (let j = 0; j < inputs.length; j++) {
            let input = inputs[j];
            if (isUsernameField(input)) {
            // if (isVisible(input) && ((input.type == "text" && (input.name == "email" || input.name == "username")) || input.type == "email" || input.autocomplete == "username" || input.autocomplete == "email")) {
                inputToField(input, "$(javascript_escape "${USERNAME}")");
            } else if (isPasswordField(input)) {
                inputToField(input, "$(javascript_escape "${PASSWORD}")");
            }
            else continue;
        }
    };
    var forms = document.getElementsByTagName("form");
    if("$(javascript_escape "${QUTE_URL}")" == window.location.href) {
        let i;
        for (i = 0; i < forms.length; i++) {
            if (hasPasswordField(forms[i])) {
                loadData2Form(forms[i].getElementsByTagName("input"));
            }
        }
        if (!i) {     // cant find a form (like fucking twitter. gotta freestyle a bitch.)
            var inputs = document.getElementsByTagName("input");
            loadData2Form(document.getElementsByTagName("input"));
        }
    } else {
        alert("Secrets will not be inserted.\nUrl of this page and the one where the user script was started differ.");
    }
EOF
}

URL=$(echo "$QUTE_URL" | awk -F/ '{print $3}' | sed 's/www.//g')

echo "message-info 'Looking for bumba password for $URL...'" >>"$QUTE_FIFO"

if op signin; then
    # find entry matching the exact host of our domain
    UUID=$(op item get "$URL")
    if [ $? -ne 0 ]; then
        TITLE=$(op item list --format=json | jq -r '.[].title' | sort | rofi -dmenu -config "$HOME/.config/rofi/scripts/1pass/1pass.rasi" -i -p "title:" -select "$URL") || TITLE=""
        if [ -n "$TITLE" ]; then
            # fallback: pick entry whose title contains the chosen string
            UUID=$(op item list --format=json |
                jq --arg title "$TITLE" -r '[.[] | select(.title | contains($title))][0].id') || UUID=""
        fi
    fi

    if [ -n "$UUID" ]; then
        ITEM=$(op item get --format=json "$UUID")

        # extract password field (fallback to any concealed field if purpose missing)
        PASSWORD=$(echo "$ITEM" | jq -r '(.fields[] | select(.purpose=="PASSWORD") | .value) // (.fields[] | select(.type=="CONCEALED") | .value)' | head -n1)

        if [ -n "$PASSWORD" ]; then
            TITLE=$(echo "$ITEM" | jq -r '.title')
            USERNAME=$(echo "$ITEM" | jq -r '.fields[] | select(.purpose=="USERNAME") | .value')

            echo "mode-enter insert" >>"$QUTE_FIFO"

            printjs() {
                js | sed 's,//.*$,,' | tr '\n' ' '
            }
            echo "jseval -q $(printjs)" >>"$QUTE_FIFO"

            echo "message-info '1P: Filled credentials for $TITLE.'" >>"$QUTE_FIFO"
            TOTP=$(op item get --otp "$UUID") || TOTP=""
            if [ -n "$TOTP" ]; then
                echo "$TOTP" | wl-copy
                echo "message-info 'Pasted one time password for $TITLE to clipboard'" >>"$QUTE_FIFO"
            fi
        else
            echo "message-error 'No password found for $URL'" >>"$QUTE_FIFO"
        fi
    else
        echo "message-error 'Entry not found for $UUID'" >>"$QUTE_FIFO"
    fi
else
    echo "message-error 'Wrong master password'" >>"$QUTE_FIFO"
fi
